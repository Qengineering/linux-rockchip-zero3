...
static struct dma_buf *system_heap_allocate_uncached_dma32(struct dma_heap *heap,
                                                           unsigned long len,
                                                           unsigned long fd_flags,
                                                           unsigned long heap_flags)
{
  ...
  int i, ret = -ENOMEM;
 	...
 	while (size_remaining > 0) {
 		...
 		page = alloc_largest_available(size_remaining, max_order);
 		if (!page) {
 			page = alloc_pages(GFP_DMA32 | __GFP_ZERO | __GFP_DMA, get_order(size_remaining));
 		}
 		if (!page)
 			goto free_buffer;
